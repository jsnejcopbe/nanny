...
<script charset="utf-8" src="http://map.qq.com/api/js?v=2.exp&key=T7LBZ-XPSRF-C7AJS-JJ7HX-SNFK7-RMBJ4"></script>
<style type="text/css">
* {
    margin: 0px;
    padding: 0px;
...
.btn, .btn_active {
    ...
    font-size: 14px;
    float: left;
    background: url(/nanny/images/index.png);
    margin-top: 14px;
    text-align: center;
    cursor: pointer;
}
...
.btn_active {
    background: url(/nanny/images/index.png) -49px 0px;
}
...
<body>
<div style="width:912px;height:570px;position:relative;margin:0px auto;">
...
qq.maps.event.addListener(map, "click", function (e) {
    ...
    url3 = encodeURI("http://apis.map.qq.com/ws/geocoder/v1/?location=" + e.latLng.getLat() + "," + e.latLng.getLng() + "&key=T7LBZ-XPSRF-C7AJS-JJ7HX-SNFK7-RMBJ4&output=jsonp&&callback=?");
...
qq.maps.event.addDomListener(btnSearch, 'click', function () {
    ...
    url = encodeURI("http://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + query_city + ",0)&page_size=9&page_index=1&key=T7LBZ-XPSRF-C7AJS-JJ7HX-SNFK7-RMBJ4&output=jsonp&&callback=?");
    $.getJSON(url, function (result) {
        ...
        if (result.count) {
            ...
            each(result.data, function (n, ele) {
                ...
                hiddenInput.value=JSON.stringify(ele);
            ...
            $("#bside_left").delegate(".info_list", "click", function (e) {
                ...
                document.getElementById("addr_cur").value = this.childNodes[2].childNodes[1].innerHTML.substring(3);
           		var result=JSON.parse(this.childNodes[0].value);
           		createBackData(result);
            });
        } else {
...
city.onclick = function (e) {
    ...
    if (target.className == "city_name") {
        ...
        url2 = encodeURI("http://apis.map.qq.com/ws/geocoder/v1/?region=" + curCity.children[0].innerHTML + "&address=" + curCity.children[0].innerHTML + "&key=T7LBZ-XPSRF-C7AJS-JJ7HX-SNFK7-RMBJ4&output=jsonp&&callback=?");
...
$(".search_t").autocomplete({
    source:function(request,response){
        url4 = encodeURI("http://apis.map.qq.com/ws/place/v1/suggestion/?keyword=" + request.term + "&region=" + curCity.children[0].innerHTML + "&key=T7LBZ-XPSRF-C7AJS-JJ7HX-SNFK7-RMBJ4&output=jsonp&&callback=?");
...
function createBackData(result){
	var data;
	if(result.hasOwnProperty("result"))
		data=result.result;
	else
		data=result;
	var info=data.ad_info;
	var pos=$("#poi_cur").val();
	var param=new Object();
	param.add_det=data.address;
	param.province=info.province;
	param.city=info.city;
	param.area=info.district;
	param.lat=pos.split(",")[0];
	param.lon=pos.split(",")[1];
	
	window.parent.getPos(param);
}

function setData(param)
{
	$("#poi_cur").val(param.lat+","+param.lon);
	$("#addr_cur").val(param.add_det);
	var center = new qq.maps.LatLng(param.lat,param.lon);
